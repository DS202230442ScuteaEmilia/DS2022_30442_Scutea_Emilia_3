/**
 * @fileoverview gRPC-Web generated client stub for grpc
 * @enhanceable
 * @public
 */

// Code generated by protoc-gen-grpc-web. DO NOT EDIT.
// versions:
// 	protoc-gen-grpc-web v1.4.2
// 	protoc              v3.19.6
// source: greet.proto


/* eslint-disable */
// @ts-nocheck


import * as grpcWeb from 'grpc-web';

import * as greet_pb from './greet_pb';


export class GreetServiceClient {
  client_: grpcWeb.AbstractClientBase;
  hostname_: string;
  credentials_: null | { [index: string]: string; };
  options_: null | { [index: string]: any; };

  constructor (hostname: string,
               credentials?: null | { [index: string]: string; },
               options?: null | { [index: string]: any; }) {
    if (!options) options = {};
    if (!credentials) credentials = {};
    options['format'] = 'text';

    this.client_ = new grpcWeb.GrpcWebClientBase(options);
    this.hostname_ = hostname.replace(/\/+$/, '');
    this.credentials_ = credentials;
    this.options_ = options;
  }

  methodDescriptorGreet = new grpcWeb.MethodDescriptor(
    '/grpc.GreetService/Greet',
    grpcWeb.MethodType.UNARY,
    greet_pb.GreetRequest,
    greet_pb.GreetResponse,
    (request: greet_pb.GreetRequest) => {
      return request.serializeBinary();
    },
    greet_pb.GreetResponse.deserializeBinary
  );

  greet(
    request: greet_pb.GreetRequest,
    metadata: grpcWeb.Metadata | null): Promise<greet_pb.GreetResponse>;

  greet(
    request: greet_pb.GreetRequest,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: greet_pb.GreetResponse) => void): grpcWeb.ClientReadableStream<greet_pb.GreetResponse>;

  greet(
    request: greet_pb.GreetRequest,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: greet_pb.GreetResponse) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.GreetService/Greet',
        request,
        metadata || {},
        this.methodDescriptorGreet,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.GreetService/Greet',
    request,
    metadata || {},
    this.methodDescriptorGreet);
  }

  methodDescriptorSendMessage = new grpcWeb.MethodDescriptor(
    '/grpc.GreetService/SendMessage',
    grpcWeb.MethodType.UNARY,
    greet_pb.Greeting,
    greet_pb.Empty,
    (request: greet_pb.Greeting) => {
      return request.serializeBinary();
    },
    greet_pb.Empty.deserializeBinary
  );

  sendMessage(
    request: greet_pb.Greeting,
    metadata: grpcWeb.Metadata | null): Promise<greet_pb.Empty>;

  sendMessage(
    request: greet_pb.Greeting,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: greet_pb.Empty) => void): grpcWeb.ClientReadableStream<greet_pb.Empty>;

  sendMessage(
    request: greet_pb.Greeting,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: greet_pb.Empty) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.GreetService/SendMessage',
        request,
        metadata || {},
        this.methodDescriptorSendMessage,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.GreetService/SendMessage',
    request,
    metadata || {},
    this.methodDescriptorSendMessage);
  }

  methodDescriptorGetMessages = new grpcWeb.MethodDescriptor(
    '/grpc.GreetService/GetMessages',
    grpcWeb.MethodType.UNARY,
    greet_pb.Empty,
    greet_pb.Messages,
    (request: greet_pb.Empty) => {
      return request.serializeBinary();
    },
    greet_pb.Messages.deserializeBinary
  );

  getMessages(
    request: greet_pb.Empty,
    metadata: grpcWeb.Metadata | null): Promise<greet_pb.Messages>;

  getMessages(
    request: greet_pb.Empty,
    metadata: grpcWeb.Metadata | null,
    callback: (err: grpcWeb.RpcError,
               response: greet_pb.Messages) => void): grpcWeb.ClientReadableStream<greet_pb.Messages>;

  getMessages(
    request: greet_pb.Empty,
    metadata: grpcWeb.Metadata | null,
    callback?: (err: grpcWeb.RpcError,
               response: greet_pb.Messages) => void) {
    if (callback !== undefined) {
      return this.client_.rpcCall(
        this.hostname_ +
          '/grpc.GreetService/GetMessages',
        request,
        metadata || {},
        this.methodDescriptorGetMessages,
        callback);
    }
    return this.client_.unaryCall(
    this.hostname_ +
      '/grpc.GreetService/GetMessages',
    request,
    metadata || {},
    this.methodDescriptorGetMessages);
  }

}

